...
def get_live_match_data(match_id: str = "latest") -> dict:
    ...
    try:
        ...
        api_key = os.environ.get("CRIC_API_KEY")
        if not api_key:
            raise Exception("No CRIC_API_KEY found")
        ...
        if matches:
            ...
            if found_match:
                ...
                rr = round(score_val / overs, 2) if overs > 0 else 0
                runs_needed = 0
                rrr = 0
                ...
                return {
                    ...
                    "data": {
                        ...
                        "batting_team": team1_abbr,
                        "bowling_team": team2_abbr,
...
def generate_commentary(match_data: dict) -> dict:
    ...
    if rrr > 14:
        ...
        line = (f"What a performance! {team1} are CRUISING — {score}/{wickets} in {overs} overs! "
                f"Running rate {match_data.get('run_rate', 0):.1f} against requirement of {rrr:.1f}. "
...
def show_tool_trace() -> dict:
    ...
    """
    tools_list = [
        "get_live_match_data", "get_match_status", "get_team_form", "get_player_stats",
